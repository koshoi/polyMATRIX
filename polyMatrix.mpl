...
iterationMatrix := proc(amtr :: Matrix) :: Vector(Boolean, Matrix);
	local i := 1;
	local j := 1;
	local iterator;
	local coef := 1;
	...
	if (not finishedColumn(mtr)) then
		...
		i := lowestDegree(mtr)[1];
		if (i = 1) then
			i := nonZero(Column(mtr, 1));
			coef := quo(mtr[i, 1], mtr[1, 1], x);
			iterator := normal(AddRow(iterator, i, 1, -coef));
		...
		j := lowestDegree(mtr)[2];
		if (j = 1) then
			j := nonZero(Row(mtr, 1));
			coef := quo(mtr[1, j], mtr[1, 1], x);
			iterator := normal(Transpose(AddRow(iterator, j, 1, -coef)));
